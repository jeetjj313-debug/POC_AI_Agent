...
# Remove spaCy imports and initialization
...
def clean_sensitive_info(text):
    ...
    # --- Alternative to spaCy NER: Use pattern-based recognition for names ---
    # Look for capitalized words that might be names (but avoid medical terms)
    # This is a simplified approach and less accurate than spaCy
    
    # First, find all potential name candidates (capitalized words)
    ...
    for candidate in name_candidates:
        ...
        for med_pattern in medicine_patterns:
            if re.search(med_pattern, candidate, re.IGNORECASE):
                is_medical = True
                break
        
        # Skip if it contains medical dosage or common medical words
        if (re.search(r'\d+(mg|mcg|g|ml|cc)', candidate, re.IGNORECASE) or 
            any(term in candidate.lower() for term in ['tab', 'gel', 'paint', 'cream'])):
            is_medical = True
        
        # Skip if it's already been processed or is clearly medical
        if (not is_medical and 
            "[" not in candidate and 
            not any(prefix.replace(r'\.?', '').lower() in text[max(0, text.find(candidate)-10):text.find(candidate)].lower() 
            ...
            context_start = max(0, text.find(candidate) - 20)
            context_end = min(len(text), text.find(candidate) + len(candidate) + 20)
            ...
            if not any(med_word in context for med_word in ['tab', 'mg', 'gel', 'paint', 'cream', 'rx', 'adv']):
                text = text.replace(candidate, "[PERSON_NAME]")
    
    # --- Remove date patterns (alternative to spaCy DATE recognition) ---
    date_patterns = [
